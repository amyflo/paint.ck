public class Pixel extends GGen {
    // initialize mesh
    GPlane pad --> this;
    FlatMaterial mat;
    pad.mat(mat);
    ...
    fun void pollHover(int note) {
        if (isHovered()) {
            onHoverEvent.broadcast();
            handleInput(MOUSE_HOVER, note);
        } else {
    ...
    fun void handleInput(int input) {
        ...
        if (state == NONE) {
            ...
            if (input == MOUSE_CLICK)      enter(NONE);
            if (input == NOTE_OFF)         enter(selected);
        }
